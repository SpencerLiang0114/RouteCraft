...
export function RouteResultsClient() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const activateSharedRoute = async () => {
      const { results: currentResults, savedRoutes, setResults, setActiveRoute: activate, loadSavedRoutes } =
      ...
      let inSaved = savedRoutes.find((r) => r.id === sharedRouteId);
      if (!inSaved) {
        await loadSavedRoutes();
        if (cancelled) return;
        inSaved = useRouteStore.getState().savedRoutes.find((r) => r.id === sharedRouteId);
      }
